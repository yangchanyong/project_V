...
public class CollectionImageService {
     ...
     * Presigned URL은 요청된 파일 크기를 Content-Length로, If-None-Match: * 를 함께 서명한다.
     * 클라이언트는 업로드 시 발급 요청과 동일한 Content-Type, 그리고 If-None-Match: * 헤더를 반드시 함께 전송해야 한다.
     *
    ...
    public PresignedUrlResponse generatePresignedUrl(Long userId, Long collectionId, PresignedUrlRequest req) {
        ...
        String presignedUrl = storageService.generatePutPresignedUrl(s3Key, req.contentType(), req.fileSize());
     ...
     * HeadObject 1회 호출로 실재 여부와 실제 크기를 함께 확인한다.
     * 신고된 fileSize가 아닌 실측 크기를 기준으로 10MB 상한을 재검증한다.
     *
     * @param userId       로그인 유저 ID
     * @param collectionId 컬렉션 PK
     * @param req          s3Key, 표시 순서
     * @return 생성된 이미지 ID
     * @throws com.chanyong.gunpla.global.exception.BusinessException FILE_UPLOAD_VALIDATION_FAILED(400) — S3에 파일 없음 또는 실제 크기 초과
     */
    @Transactional
    public ImageSaveResponse saveImage(Long userId, Long collectionId, ImageSaveRequest req) {
        ...
        OptionalLong actualSize = storageService.findObjectSize(req.s3Key());
        if (actualSize.isEmpty() || actualSize.getAsLong() > MAX_FILE_SIZE) {
